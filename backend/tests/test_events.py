...
def seed_events(app):
    ...
    with app.app_context():
        club = Club(club_name="Chess Club", description="Competitive and casual chess play", status='approved')
        db.session.add(club)
        db.session.commit()
        e1 = Event(
...
def test_browse_events_by_club_tc016(client, auth_headers, app):
    ...
    """
    with app.app_context():
        club_a = Club(club_name="Chess Club", description="Strategy games", status='approved')
        club_b = Club(club_name="Robotics Society", description="Bots and drones", status='approved')
        ...
        e1 = Event(club_id=club_a.club_id, event_name="Blitz Night",
                    ...
                    location="Room 204", status='approved')
        e2 = Event(club_id=club_a.club_id, event_name="Chess Tournament",
                    ...
                    location="Gymnasium", status='approved')
        e3 = Event(club_id=club_b.club_id, event_name="Drone Build Day",
                    ...
                    location="Engineering Lab", status='approved')
